...
import { img1, img2, img3, img4, img5, img6, img7, img8 } from "../../assets/images/rems";

const features = [
  "Ballot Management for automated property allocation",
  "Project Management to plan and track milestones",
  "Property Booking with payment plans and invoices",
  "Accounting for financial management and reports",
  "Procurement for vendor and purchase management",
  "Bank Transactions for accurate reconciliation",
  "Subscription-based scalability",
  "Custom branding and multi-tenancy",
];
      ...
      <section id="heading" className="relative min-h-[65vh] flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="section-tag">
            <BiSolidBuildingHouse size={14} /> Software Product
          </motion.span>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="text-4xl sm:text-5xl lg:text-6xl font-bold font-heading text-white mt-4 mb-6">
            Real Estate <span className="gradient-text-cobalt">Management System</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="text-titanium text-lg max-w-3xl mx-auto leading-relaxed">
            The Real Estate Management ERP is a cloud-based SaaS platform designed to streamline operations with advanced multi-tenancy, branch-wise configurations, and role-based access control.
          </motion.p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              ...
              <h2 className="text-2xl sm:text-3xl font-bold font-heading text-white mb-4">What is REMS?</h2>
              <p className="text-titanium leading-relaxed mb-8">
                With powerful reporting, subscription-based scalability, and custom branding, this all-in-one solution enhances efficiency, transparency, and growth for real estate businesses. Manage everything from property allocation to complex vendor procurement.
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <div className="liquid-glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: "Cloud SaaS", val: "✓" }, { label: "Multi-tenancy", val: "✓" },
                    { label: "Branch Config", val: "✓" }, { label: "Role Access", val: "✓" },
                    { label: "Ballot Mgmt", val: "✓" }, { label: "Accounting", val: "✓" },
                  ].map((item) => (
            ...
            {[img1, img2, img3, img4, img5, img6, img7, img8].map((img, i) => (
              <div key={i} className="px-2">
                <div className="rounded-2xl overflow-hidden cobalt-glow-border p-1">
                  <img src={img} alt={`REMS Screenshot ${i + 1}`} className="w-full rounded-xl object-cover" />
                </div>
              </div>
            ))}
          </Slider>
