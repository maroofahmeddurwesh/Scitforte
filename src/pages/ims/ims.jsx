...
import { img13, img14, img15, img16 } from "../../assets/images/osprey";
...
const features = [
  ...
  "Automated Workflows",
  "Enhanced Productivity Dashboards",
];
...
const InstituteManagementSystem = () => {
  return (
      ...
      <section id="heading" className="relative min-h-[65vh] flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="section-tag">
            <GrSystem size={14} /> Software Product
          </motion.span>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="text-4xl sm:text-5xl lg:text-6xl font-bold font-heading text-white mt-4 mb-6">
            Osprey <span className="gradient-text-cobalt">IMS & ERP</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="text-titanium text-lg max-w-3xl mx-auto leading-relaxed">
            A comprehensive business management solution designed to streamline operations, centralize processes, and automate workflows.
          </motion.p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              ...
              <h2 className="text-2xl sm:text-3xl font-bold font-heading text-white mb-4">What is Osprey?</h2>
              <p className="text-titanium leading-relaxed mb-8">
                Osprey is a modern Institute Management System and Enterprise Resource Planning tool that enhances productivity for businesses by bringing all your essential modules into one unified platform.
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <div className="liquid-glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: "Centralized Data", val: "✓" }, { label: "Work Mgmt", val: "✓" },
                    { label: "CRM Integrated", val: "✓" }, { label: "Finance module", val: "✓" },
                    { label: "HR module", val: "✓" }, { label: "Productivity", val: "✓" },
                  ].map((item) => (
      ...
      <section className="pb-20 bg-obsidian-dark relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-12">
          ...
          <Slider {...sliderSettings}>
            {[img13, img14, img15, img16].map((img, i) => (
              <div key={i} className="px-2">
                <div className="rounded-2xl overflow-hidden cobalt-glow-border p-1">
                  <img src={img} alt={`Osprey Screenshot ${i + 1}`} className="w-full rounded-xl object-cover" />
                </div>
              </div>
            ))}
          </Slider>
...
export default InstituteManagementSystem;