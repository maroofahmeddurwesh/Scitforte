...
import { img9, img10, img11, img12 } from "../../assets/images/livestock";

const features = [
  "Tag ID generation and tracking",
  "Mandi expense tracking",
  "Animal routing (quarantine, lairage, hospital)",
  "Procurement & Inventory (medicines, vaccines, feed)",
  "Custom feed mixtures & weight monitoring",
  "Vaccination schedules management",
  "Slaughter management and reporting",
  "Meat and offal sales tracking",
];
      ...
      <section id="heading" className="relative min-h-[65vh] flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="section-tag">
            <FaBuildingUser size={14} /> Software Product
          </motion.span>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="text-4xl sm:text-5xl lg:text-6xl font-bold font-heading text-white mt-4 mb-6">
            Livestock Fattening <span className="gradient-text-cobalt">ERP</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="text-titanium text-lg max-w-3xl mx-auto leading-relaxed">
            The Livestock Fattening ERP streamlines the livestock fattening process, from tag ID generation and mandi expense tracking to animal routing and sales.
          </motion.p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              ...
              <h2 className="text-2xl sm:text-3xl font-bold font-heading text-white mb-4">What is Livestock ERP?</h2>
              <p className="text-titanium leading-relaxed mb-8">
                It manages procurement, inventory, custom feed mixtures, weight monitoring, and vaccination schedules. Animals are eventually sent for slaughter, with products like meat and offal sold to customers. This system ensures efficient tracking, cost management, and profitability for livestock operations.
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <div className="liquid-glass rounded-2xl p-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: "Tag ID Tracking", val: "✓" }, { label: "Expense Track", val: "✓" },
                    { label: "Inventory Mgmt", val: "✓" }, { label: "Vaccinations", val: "✓" },
                    { label: "Slaughter Mgmt", val: "✓" }, { label: "Sales & Profit", val: "✓" },
                  ].map((item) => (
      ...
      <section className="pb-20 bg-obsidian-dark relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-12">
          ...
          <Slider {...sliderSettings}>
            {[img9, img10, img11, img12].map((img, i) => (
              <div key={i} className="px-2">
                <div className="rounded-2xl overflow-hidden cobalt-glow-border p-1">
                  <img src={img} alt={`ERP Screenshot ${i + 1}`} className="w-full rounded-xl object-cover" />
                </div>
              </div>
            ))}
          </Slider>
